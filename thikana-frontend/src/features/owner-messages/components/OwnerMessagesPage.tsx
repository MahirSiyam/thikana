...
import { messageConversations as initialConversations } from "@/features/owner-messages/data/owner-messages.mock";
import type {
  ChatMessage,
  MessageConversation,
} from "@/features/owner-messages/types/owner-messages.types";
...
function ConversationRow({
  ...
  return (
    <button
      type="button"
      onClick={onSelect}
      className={`relative flex h-[70px] w-full items-center gap-3 border-b border-[#f0f0ee] px-4 text-left transition-colors ${
        selected ? "bg-[#f3f4f6]" : "bg-white hover:bg-[#fafaf9]"
      }`}
    >
      {selected ? (
        <span
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-0.5">
        <div className="flex items-center gap-2">
          ...
          <span className="shrink-0 rounded bg-[#f3f4f6] px-1.5 py-0.5 font-inter text-[10px] font-semibold text-[#6b7280]">
            {conversation.role}
          </span>
        ...
        <div className="flex items-center gap-2">
          ...
          {conversation.unread ? (
            <span
              aria-label="Unread"
              className="size-2 shrink-0 rounded-full bg-[#ef4444]"
            />
          ) : null}
        </div>
      </div>
    ...
    <section className="flex min-h-[520px] min-w-0 flex-1 flex-col overflow-hidden bg-white lg:min-h-[896px]">
      <div className="flex h-16 shrink-0 items-center justify-between gap-3 border-b border-[#e5e5e2] px-4 sm:px-6">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="flex min-w-0 flex-col gap-0.5">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="rounded bg-[#f3f4f6] px-1.5 py-0.5 font-inter text-[10px] font-semibold text-[#6b7280]">
                {conversation.role}
              </span>
            </div>
            {conversation.online ? (
        ...
        {conversation.subject ? (
          <span className="hidden shrink-0 items-center gap-1.5 rounded-lg bg-[#fef3c7] px-2.5 py-1.5 font-inter text-[11px] font-semibold text-[#f59e0b] sm:inline-flex">
            <Image
              src="/images/owner/icon-house.svg"
              alt=""
              width={12}
              height={12}
              aria-hidden="true"
              ...
              <div
                className={`max-w-[400px] p-3 font-inter text-[13px] leading-normal ${
                  isMine
                    ...
                    : "rounded-bl rounded-br-xl rounded-tl-xl rounded-tr-xl bg-[#f3f4f6] text-brand-dark"
                }`}
              >
                {message.body}
              </div>
      ...
      >
        ...
        <label className="sr-only" htmlFor="owner-message-input">
          Type a message
        </label>
        <input
          ...
          className="h-11 min-w-0 flex-1 rounded-[22px] bg-[#f3f4f6] px-4 font-inter text-[13px] text-brand-dark outline-none placeholder:text-[#6b7280] focus-visible:ring-2 focus-visible:ring-brand-dark"
        />
        <button
          type="submit"
          aria-label="Send message"
          className="flex size-11 shrink-0 items-center justify-center rounded-full bg-black focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-dark focus-visible:ring-offset-2"
        >
          <Image
      ...
      <div className="flex w-full flex-col gap-6">
        <header className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex h-10 w-full max-w-[309px] items-center gap-2 rounded-[20px] bg-[#f5f5f3] px-4">
            ...
            />
            <label className="sr-only" htmlFor="owner-messages-search">
        ...
        <div className="flex min-h-0 flex-col overflow-hidden rounded-2xl border border-[#e5e5e2] bg-white lg:flex-row">
          <aside
            className={`w-full shrink-0 border-[#e5e5e2] lg:w-[292px] lg:border-r ${
              mobileShowChat ? "hidden lg:flex lg:flex-col" : "flex flex-col"
            }`}
          >
            <div className="p-4">
              <div className="flex h-9 items-center gap-2 rounded-full bg-[#f3f4f6] px-3">
                <Image
                  src="/images/owner/icon-search.svg"
                  alt=""
                  width={14}
                  ...
                  className="size-3.5 shrink-0"
                />
                <label className="sr-only" htmlFor="owner-chats-search">
                  Search conversations
                </label>
                <input
                  id="owner-chats-search"
                  type="search"
                  value={chatQuery}
