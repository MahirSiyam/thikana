  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (
        ...
        !me.registrationComplete
      ) {
        await signOut(adminAuth);
        setError(
          "This account is not an approved admin. Run `npm run seed:admin` in the backend."
        );
        return;
      }
    ...
    <div className="w-full max-w-[466px] rounded-xl bg-white px-4 py-4 shadow-[4px_4px_5px_rgba(10,10,10,0.1)] sm:px-6 sm:py-5">
      <header className="mb-3 space-y-0.5">
        ...
        <p className="font-inter text-sm text-brand-dark/60">
          Separate admin portal — not linked to tenant/owner accounts.
        </p>
      </header>
      ...
      <form className="flex flex-col gap-3" onSubmit={handleSubmit}>
        <div className="flex flex-col gap-1.5">
          <label
            ...
            className="font-inter text-base font-medium text-brand-dark"
          >
            Admin email
          </label>
          <div className="flex h-11 items-center gap-3 rounded-lg border-[0.5px] border-brand-dark bg-white px-3 focus-within:ring-2 focus-within:ring-brand-dark/20">
            ...
            <input
              ...
              placeholder="admin@thikana.local"
